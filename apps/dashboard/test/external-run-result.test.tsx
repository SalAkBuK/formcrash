...
describe('external run outcome-first result', () => {
  it('leads with the failed exact-once outcome and keeps technical evidence secondary', async () => {
    ...
    const expectedObserved = screen.getByRole('heading', {
      name: 'Expected versus observed',
    });
    expect(
      headline.compareDocumentPosition(expectedObserved) &
        Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
    expect(
      screen.getByText('Exactly 1 visible matching result.'),
    ).toBeVisible();
    expect(
      screen.getAllByText('2 visible matching results').length,
    ).toBeGreaterThan(0);
    ...
    expect(
      screen.getByRole('heading', { name: 'Individual Outcome Checks' }),
    ).toBeVisible();
  ...
  it('renders the passed fixture with the approved description and observed count one', () => {
    ...
    expect(
      screen.getAllByText('1 visible matching result').length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(
        'One visible result matched the approved generated identity.',
      ),
    ).toBeVisible();
  });
  ...
  it('keeps runner failures distinct from application outcome failures', () => {
    ...
    expect(
      screen.getByText('The saved journey step did not complete.'),
    ).toBeVisible();
    expect(
      screen.queryByText(
...
function run(
  kind: 'failed' | 'passed' | 'legacy' | 'runner_error',
): ExternalRunDetail {
  const hasOutcome = kind !== 'legacy';
  const isRunnerError = kind === 'runner_error';
  const observedCount = kind === 'failed' ? 2 : kind === 'passed' ? 1 : null;
  ...
  return externalRunDetailSchema.parse({
    ...
    outcomeAggregate:
      kind === 'legacy'
        ...
        : isRunnerError
          ? 'could_not_verify'
          : kind,
    assertionAggregate: isRunnerError ? 'could_not_verify' : 'passed',
    ...
    networkObservations: [requestObservation()],
    assertions: [technicalAssertion()],
